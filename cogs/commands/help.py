...
class HelpCommand(commands.HelpCommand):
  ...
  async def send_bot_help(self, mapping):
    ...
    embed = discord.Embed(
      ...
      description=
      f"Prefix for this server is `$`\nTotal Commands: {len(set(self.context.bot.walk_commands()))} | Usable by you (here): {len(set(filtered))}\n [Invite]({inv}) | [Support Server](https://discord.gg/WBgDeWBPwG) | [Vote Me](Comming Soon)\nType `$help <command | module>` for more info regarding it.",
      color=0x2f3136)
  ...
  async def send_cog_help(self, cog):
    ...
    if str(self.context.author.id) in data["ids"]:
      embed = discord.Embed(
        title="<:error_ok:1002376341959757884> Blacklisted",
        description=
        "You Are Blacklisted From Using My Commands.\nIf You Think That It Is A Mistake, You Can Appeal In Our Support Server By Clicking [here](https://discord.gg/WBgDeWBPwG)",
        color=0x2f3136)
      #return await self.context.reply(embed=embed, mention_author=False)
      return None
    elif str(self.context.channel.id) in randi["ids"]:
