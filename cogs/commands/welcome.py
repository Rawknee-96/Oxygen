...
from utils.Tools import blacklist_check,ignore_check,updateDB,getDB
...
```""",
  ...
  async def greet_image_error(self, ctx, error):
    if isinstance(error, commands.CheckFailure):
        embed = discord.Embed(
            description="This command is for premium users only. Click [here](https://discord.gg/WBgDeWBPwG) to subscribe to premium.",
            color=0x2f3136
        )
        button1 = discord.ui.Button(label="Premium",
                               style=discord.ButtonStyle.url,
                               url="https://discord.gg/WBgDeWBPwG")
        view = discord.ui.View()
        view.add_item(button1)
        await ctx.send(embed=embed, view=view)
