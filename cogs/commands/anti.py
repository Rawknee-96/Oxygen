...
import discord
from utils.Tools import blacklist_check,ignore_check,getanti,getConfig,getHacker,updateanti,updateHacker,updateConfig
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(wled) == 15:
                hacker = discord.Embed(
                    ...
                    description=
                    f"<:anxAlert:1108101889687560336> This server has already reached the maximum number of whitelisted users (15).\nPlease remove one to add another.",
                    color=0x2f3136)
                await ctx.reply(embed=hacker, mention_author=False)
            else:
